...
def eval_seq(opt, gpn, dataloader, data_type, result_filename, evaluator, writer, n_iter, save_dir=None, show_image=True, frame_rate=30):
    ...
    tracker = JDETracker(opt, frame_rate=frame_rate, train=True)
    timer = Timer()
    results = []
    frame_id = 0
    plot_arguments = []
    ...
    for path, img, img0 in dataloader:
        ...
        if opt.ghost_stats:
            online_targets, n_iter, ghosts, ghost_match_iou = tracker.update(blob, img0, opt, evaluator, writer, n_iter)
            ghost_tlwhs = [g.tlwh for g in ghosts]
            ghost_sequence.append((ghost_tlwhs, frame_id))
            ghost_match_ious.extend(ghost_match_iou)

        else:
            online_targets, n_iter = tracker.update(blob, img0, opt, evaluator, writer, n_iter)
...
def main(opt, data_root='/data/MOT16/train', det_root=None, seqs=('MOT16-05',), exp_name='demo',
    ...
    for i, seq in enumerate(seqs):
        ...
        if opt.ghost_stats:
            nf, ta, tc, n_iter, ghost_sequence, ghost_match_ious = eval_seq(opt, dataloader, data_type, result_filename, evaluator, writer, n_iter,
